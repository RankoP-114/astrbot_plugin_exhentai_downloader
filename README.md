...
- 搜索统计：结果标题会区分当前页条目数和站点返回的总结果数，默认展示当前页最多 25 条。
...
| `private_admin_only` | 私聊仅 AstrBot 管理员可用；即使关闭全局 `admin_only` 也会生效 | `false` |
...
- `private_admin_only` 可单独限制私聊入口，开启后私聊只允许 AstrBot 管理员使用。
